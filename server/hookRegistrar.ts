...
export const COMMANDS: CommandSpec[] = [
  ...
  },
  {
    name: 'cortex-sync',
    description:
      'Full Cortex sync: capture this session into memory, fold it into the local graph, then bidirectionally sync the private mind (push local, pull + adopt remote) and report both ways.',
    body: [
      'Full Cortex sync — the LOUD, intentional sync. Two phases: capture this session, then reconcile with the private mind. Run it when you want this machine published and refreshed. (A silent per-turn background sync already keeps the LOCAL graph current; this command adds the session capture and the REMOTE reconcile.)',
      '',
      '## 1. Capture this session',
      '',
      'Do exactly what /cortex-snapshot does: review the conversation and write each worth-remembering item as a memory `.md` file (with `name` / `description` / `type: memory` frontmatter) under the right scope (`~/.claude/memory/` or the project memory dir). Don\'t skip this — the sync publishes memory FILES, so anything not written is not synced.',
      '',
      '## 2. Fold into the LOCAL graph',
      '',
      '1. Run: `' + path.join(PROJECT_ROOT, 'node_modules', '.bin', 'tsx') + ' ' + path.join(PROJECT_ROOT, 'bin', 'ckn-sync.ts') + '`',
      '2. This folds the just-written `.md` into this machine\'s graph so recall sees them immediately. The running server queries the LIVE graph, so the machine is fresh with NO restart.',
      '',
      '## 3. Sync the private mind (bidirectional)',
      '',
      '1. Run: `' + path.join(PROJECT_ROOT, 'node_modules', '.bin', 'tsx') + ' ' + path.join(PROJECT_ROOT, 'bin', 'ckn-mind-sync.ts') + '`',
      '2. This PUSHES local-unsynced memories to the remote, then PULLS and ADOPTS remote memories into this machine\'s graph (re-indexed in place — again, no restart). It is the ONLY step that touches the remote; the per-turn background sync is local-only.',
      '3. If it reports "no remote configured", the private mind is not enabled (it is opt-in) — phases 1-2 still applied locally. Tell the user they can enable it once with `ckn-mind-sync --remote <git-url>`.',
      '',
      '## 4. Report both ways',
      '',
      'Summarize from the script output: how many memories were PUSHED to the remote, how many were ADOPTED from the remote (and any codegraph replay or possible-duplicate notes). State plainly that this machine is now fresh — no restart needed.',
      '',
      'Note: other machines are NOT force-updated by your push. A peer adopts these changes only when IT runs /cortex-sync or boots fresh. That pull-on-intent is by design (each node controls when it adopts), not a gap.',
    ].join('\n'),
  },
  {
    name: 'cortex-rename',

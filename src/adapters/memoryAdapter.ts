...
import { serializeMemory } from './memorySerialize'
...
export const writeMemoryEntry = async (
  ...
  // Preserve the original frontmatter (id, the normalizer's nested metadata
  // block, machine lineage, pinned, …) — the editor only models name/
  // description/type/body, so writing just those would destroy everything else
  // (e.g. flatten a thread into a dead memory). See serializeMemory.
  const serialized = serializeMemory(
    { name: next.name, description: next.description, type: next.type },
    next.body,
    original?.raw,
  )
  await fs.writeText(nextPath, serialized)

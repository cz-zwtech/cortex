...
export const permissionSpec: KindSpec<Permission> = {
  kind: 'permission',
  label: 'Permission',
  pluralLabel: 'Permissions',
  schema: Permission,
  validScopes: ['user', 'project'],
  idOf: (v) => `${v.mode}::${v.index}`,
  nameOf: (v) => v.pattern || `(empty ${v.mode})`,
  searchText: (v) => `${v.mode} ${v.pattern}`.toLowerCase(),
}
...
export const symbolSpec: KindSpec<CodeSymbol> = {
  kind: 'symbol',
  label: 'Symbol',
  pluralLabel: 'Symbols',
  schema: CodeSymbol,
  // Graph-backed (not FS-scoped). Surfaced under user scope so it appears in
  // the sidebar/palette without a project root; read-only + noCreate since
  // symbols are derived data ingested via /api/graph/symbols/upsert.
  validScopes: ['user'],
  readOnly: true,
  noCreate: true,
  idOf: (v) => v.id,
  nameOf: (v) => v.name,
  searchText: (v) =>
    `${v.name} ${v.symbolKind} ${v.repo} ${v.file} ${v.signature ?? ''}`.toLowerCase(),
}
